...
use crate::{AppState, app::idem_keys::controllers, nats::config::handle_processing_error};

pub async fn start_idem_processor(state: web::Data<AppState>) {
    tokio::spawn(async move {
        let mut retry_count = 0;
        let max_retries = 5;

        loop {
            match state.streamer.consume_from_stream("IDEMS", 10).await {
                Ok(messages) => {
                    retry_count = 0;
